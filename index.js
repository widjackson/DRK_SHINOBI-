	...
	if (pairingCode && !XeonBotInc.authState.creds.registered) {
		...
		async function getPhoneNumber() {
			...
			if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v)) && !phoneNumber.length < 6) {
				...
				console.log(chalk.bgBlack(chalk.redBright('Start with your Country WhatsApp code') + chalk.whiteBright(',') + chalk.greenBright(' Example : 509xxx')));
...
app.get('/', (req, res) => {
  res.send('Hello ㊙️𝙎-𝙏𝞢𝞜!')
})

app.listen(port, () => {
  console.log(`DRK_SHINOBI is listening on port ${port}`)
})
