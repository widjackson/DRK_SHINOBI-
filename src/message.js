		...
		for (let n of update) {
			...
			if (setinfo) {
				...
				try {
					...
					profile = 'https://i.imgur.com/ry5vzGW.jpeg';
				}
				if (n.announce) {
		...
		if (welcome) {
			...
			for (let n of participants) {
				...
				try {
					...
					profile = 'https://i.imgur.com/bsxE6cT.jpeg';
				}
				XeonWlcm = await getBuffer(profile)
                XeonLft = await getBuffer(profile)
				if (action == 'add') {
				let xeonName = n
                const xdate = moment.tz('America/Port-au-Prince').locale('en-PK').format('DD/MM/YYYY');
...
await XeonBotInc.relayMessage(id, msgs.message, {})
				...
				let xeonName = n
                const xeondate = moment.tz('America/Port-au-Prince').locale('en-PK').format('DD/MM/YYYY');
                const xeontime = moment().tz('America/Port-au-Prince').locale('en-PK').format('HH:mm:ss');
...
async function MessagesUpsert(XeonBotInc, message, store) {
	try {
		...
		require('../GlobalMD')(XeonBotInc, m, message, store);
